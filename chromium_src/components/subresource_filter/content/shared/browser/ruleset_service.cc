...
void IndexedRulesetLocator::DeleteObsoleteRulesets(
  ...
  for (base::FilePath format_dir = format_dirs.Next(); !format_dir.empty();
    ...
    LOG(ERROR) << "SubresourceFilter [DeleteObsoleteRulesets]: "
               << "format_dir: " << format_dir.value()
               << ", current_format_dir: " << current_format_dir.value();
...
void RulesetService::OpenAndPublishRuleset(
    const IndexedRulesetVersion& version) {
  LOG(ERROR) << "SubresourceFilter [OpenAndPublishRuleset]: Version" << version.content_version;
  ...
  LOG(ERROR) << "SubresourceFilter [OpenAndPublishRuleset]: Opening filepath" << file_path;
  if (!base::PathExists(file_path)) {
    LOG(ERROR) << "SubresourceFilter [OpenAndPublishRuleset]: File does not exist";
    return;
  }
...
void RulesetService::OnRulesetSet(RulesetFilePtr file) {
  ...
  LOG(ERROR) << "SubresourceFilter [OnRulesetSet]: Checking file validity";
  if (!file->IsValid()) {
    LOG(ERROR) << "SubresourceFilter [OnRulesetSet]: File is not valid";
    IndexedRulesetVersion(config_.filter_tag).SaveToPrefs(local_state_);
    return;
  }
  LOG(ERROR) << "SubresourceFilter [OnRulesetSet]: File is valid, publishing new ruleset version";
